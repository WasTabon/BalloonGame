...
public class SetupGameScene
{
    ...
    public static void Setup()
    {
        ...
        var balloonSprite = GetOrCreateCircleSprite("BalloonCircle", 128);
        var shieldSprite = GetOrCreateCircleSprite("ShieldCircle", 128);

        var cam = SetupCamera();
        var balloon = SetupBalloon(balloonSprite);
        var shield = SetupShield(shieldSprite);
        SetupCameraTarget(cam, balloon.transform);
        var gameplayManager = SetupGameplayManager(balloon, shield);
        SetupGameCanvas(gameplayManager);

        Debug.Log("[Iteration 3] Game scene setup complete!");
    }
    ...
    private static GameplayManager SetupGameplayManager(Balloon balloon, Shield shield)
    {
        var go = FindOrCreate("GameplayManager");
        var gm = EnsureComponent<GameplayManager>(go);

        var so = new SerializedObject(gm);
        so.FindProperty("balloon").objectReferenceValue = balloon;
        so.FindProperty("shield").objectReferenceValue = shield;
        so.ApplyModifiedProperties();

        return gm;
    }

    private static void SetupGameCanvas(GameplayManager gameplayManager)
    {
    ...
    private static PausePopup SetupPausePopup(GameObject canvasGo)
    {
        ...
        resumeColors.highlightedColor = new Color(0.35f, 0.8f, 0.45f);
        resumeColors.pressedColor = new Color(0.25f, 0.6f, 0.35f);
        ...
        menuColors.highlightedColor = new Color(0.7f, 0.3f, 0.3f);
        menuColors.pressedColor = new Color(0.5f, 0.2f, 0.2f);
